...
export default function CandidatesTab({
  ...
  useEffect(() => {
    const loadCandidateProfile = async () => {
      // Try to get candidate_profile column first, then fallback to preferences
      const { data: profile, error } = await supabase
        .from('profiles')
        .select('candidate_profile, preferences')
        .eq('user_id', id)
        .single()
  ...
  const patchCandidateData = async (stepNumber: number) => {
    ...
    const existingData = currentProfile?.candidate_profile ||
      currentProfile?.preferences?.candidate_profile || {
        general: {},
        core: {},
        policies: {},
        responsibility: {},
    ...
    const result = await saveCandidateProfile(supabase, id, updatedData)
    return result
  }
